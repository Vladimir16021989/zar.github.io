		...
		if($u->tfer['finish1']==0 && $u->tfer['finish2']==0)
		{
			...
			}elseif(isset($_POST['start2']))
			{
				...
				if($u->tfer['good1']>0 && $u->tfer['good2']>0)
				{
					...
					//Обмениваем предметы
					$sp = mysql_query('SELECT `u`.*,`m`.`price1`,`m`.`price2`,`m`.`name` FROM `items_users` AS `u` LEFT JOIN `items_main` AS `m` ON `m`.`id` = `u`.`item_id` WHERE (`u`.`uid`="'.$u->tfer['uid1'].'" OR `u`.`uid`="'.$u->tfer['uid2'].'") AND `u`.`delete`="0" AND `u`.`inOdet`="0" AND `u`.`inShop`="0" AND `u`.`inTransfer` > "0"');
					...
					while($pl = mysql_fetch_array($sp))
					{
						...
						$upd = mysql_query('UPDATE `items_users` SET `uid` = "'.$pl['uid'].'",`gift` = "'.$pl['gift'].'" WHERE `id` = "'.$pl['id'].'" AND `inShop` = "0" AND `delete` < "1234567891" AND `inOdet` = "0"');
						if($upd)
						{
					...
					$upd1 = mysql_query('UPDATE `items_users` SET `inTransfer` = "0" WHERE (`uid`="'.$u->tfer['uid1'].'" OR `uid`="'.$u->tfer['uid2'].'") AND `delete` < "1234567891" AND `inOdet`="0" AND `inShop`="0" AND `inTransfer` > "0"');
					if($upd1 && $upd2)
					{
			...
			if($_POST['id']=='minireflesh' || $_POST['id']=='reflesh')
			{
				...
				if($u->tfer['r'.$f]!=$u->tfer['r0'])
				{
					function itmInfotf($pl,$cl)
					{
						...
						if($pl['uid']==$u->info['id'])
						{
							$r .= '<img width="13" height="13" onClick="cancelitm('.$pl['id'].');" class="clr" src="http://img.combatz.ru/i/clear.gif" />';
						}
						$r .= '<a href="items_info.php?id='.$pl['item_id'].'&rnd='.$code.'" target="_blank">'.$pl['name'].''.$x.'</a><br>';
						if($pl['inTransfer']==1)
						{
							$r .= '<img width="16" height="18" title="Этот предмет будет подарен" src="http://img.combatz.ru/i/podarok.gif" />';
						}else{
							$r .= '<small style="font-size:10px">(налог: 1кр.)</small>';
						}
						$r = '<table width="100%" border="0" cellspacing="0" cellpadding="5"><tr><td width="50" align="center"><img src="http://img.combatz.ru/i/items/'.$pl['img'].'" class="tfii"/></td><td valign="top" class="tfid">'.$r.'</td></tr></table>';

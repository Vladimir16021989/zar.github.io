	...
	while($i <= count($tm_v)) {
		...
		while($j < count($tm_u[$tm_v[$i]])) {
			...
			if($us['id'] > 0) {
				...
				if( $us['hp'] < 1 ) {
					...
					$winw = '<img title="Погиб" width="7" height="7" src="http://img.combatz.ru/i/ico/looses.gif">';
				}else{
					$winw = '<img title="Выжил" width="7" height="7" src="http://img.combatz.ru/i/ico/wins.gif">';
				}
		...
		if( $team_data['g'] == true ) {
			...
			if( $team_data['hp'] < 1 ) {
				...
				$winw = '<img src="http://img.combatz.ru/i/flag.gif" width="20" height="20" title="Победитель">';
			}
...
<body bgcolor="#E2E0E0">
<H3><IMG SRC="http://img.combatz.ru/i/fighttype2.gif" WIDTH=20 HEIGHT=20> Бойцовский клуб<? if( $based != 'battle_logs' ) { echo ' (Архив поединков)'; } ?> &nbsp; <a href="http://www.combatz.ru/">www.combatz.ru</a> <IMG SRC="http://img.combatz.ru/i/fighttype2.gif" WIDTH=20 HEIGHT=20></H3>
<? echo $r; ?>
</body>
</html>
<?php
	...
	{
		die(json_encode(array('js'=>'top.location="club.php?exit='.$code.'";')));
...
//Отправляем сообщения
	if(isset($_POST['msg']) && str_replace(' ','',$_POST['msg'])!='')
	{
		...
		function tolink($buf) {
			...
			for ($j=0; $j<count($x); $j++) {
			 if (preg_match
				("/(http:\\/\\/)?([a-z_0-9-.]+\\.combatz+\\.ru(([ \"'>\r\n\t])|(\\/([^ \"'>\r\n\t]*)?)))/",
				$x[$j],$ok))
				$newbuf.=str_replace($ok[2],"<small><a href=http://$ok[2] target=_blank ><i>[Внутреняя ссылка]</i></a></small>",
		...
		}elseif($chat->str_count($_POST['msg'],4)=='/dnd')
		{
			...
			*/
			...
			if($u->info['admin'] == 0) {
				...
				if($msg['spam'] == 1 && $u->info['molch1']<time())
				{
			...
			*/
			...
			if(isset($qix['id'])) {
				...
				$msg['cancel']++;
			}elseif($msg['type'] == 3 && $u->info['activ'] > 0)
			{
				$r['js'] .= 'chat.sendMsg(["new","'.time().'","6","","'.$u->info['login'].'","<small>Отправлять сообщения в <b>приват</b> могут только персонажи подтвердившие свой e-mail.</small>","Black","1","1","0"]);';
				$msg['cancel']++;
			}elseif($msg['type'] == 3 && $u->info['level']<0)
			{
				$r['js'] .= 'chat.sendMsg(["new","'.time().'","6","","'.$u->info['login'].'","<small>Отправлять сообщения в <b>приват</b> могут персонажи старше нулевого уровня.</small>","Black","1","1","0"]);';
				$msg['cancel']++;
			...
			if($msg['cancel']==0)
			{
				...
				if($nosend == 0) {
					...
					if( mb_strtolower($msg['to']) == 'баба маня' ) {
						...
						if( $u->info['id'] == 1012655 ) {
							...
							if( isset($lmv['id']) ) {
								...
								$imn = 0; $imgd = 0;
								$imna = explode(',',mb_strtolower( $lmv['answer'] ));
								while($imn < count($imna)) {
									if( mb_strtolower(trim($imna[$imn],' ')) == mb_strtolower( trim($_POST['msg'],' ') ) ) {
										$imgd = 1;
									}
									$imn++;
								}
								if( mb_strtolower( $lmv['answer'] ) == mb_strtolower( trim($_POST['msg'],' ') ) || $imgd == 1 ) {
									//правильный ответ
									...
									//не правильный ответ
									//$mntext = '<b>'.$u->info['login'].'</b>, '.mb_strtolower( trim($_POST['msg'],' ') ).'!';
									//mysql_query('INSERT INTO `chat` (`text`,`city`,`login`,`to`,`type`,`new`,`time`,`room`) VALUES ("'.mysql_real_escape_string($mntext).'","capitalcity","Баба Маня","","1","1","'.time().'","3")');
								}
							}
						}
...
//Получаем список онлайн
	if($_GET['r1']!=0)
	{
		$sp = mysql_query('SELECT `u`.`sex`,`s`.`dnow`,`u`.`timeMain`,`s`.`bot`,`s`.`atack`,`u`.`afk`,`u`.`dnd`,`u`.`banned`,`u`.`molch1`,`u`.`room`,`u`.`id`,`u`.`city`,`u`.`cityreg`,`u`.`online`,`u`.`align`,`u`.`clan`,`u`.`login`,`u`.`level`,`u`.`inTurnir`,`u`.`battle`,`u`.`invis`,`u`.`inUser`,`s`.`x`,`s`.`y` FROM `users` AS `u` LEFT JOIN `stats` AS `s` ON `s`.`id` = `u`.`id` WHERE ((`u`.`room` = "'.$u->info['room'].'" AND "'.mysql_real_escape_string((int)$_GET['r3']).'" != "1") OR ("'.mysql_real_escape_string((int)$_GET['r3']).'" = "1" AND `pass` NOT LIKE "%saint%")) AND `mail`!="No E-mail" AND `room` != "303" ORDER BY `u`.`inUser` DESC,`u`.`online` DESC,`u`.`login` DESC LIMIT 700');
		$ar_id = 0;
		$ar_lvl = 0;
		$ar_all = 0;
		$usl = array();
		...
		while($pl = mysql_fetch_array($sp)) {
			...
			if( ($pl['online'] > time()-520 || $pl['inUser'] > 0) && $pl['bot']!=1 && $pl['banned'] == 0 && $ysee == 1)
			{
				$usid[$pl['login']] = true;	
		...
		//Пещерное сообщение
			...
			if($see == 1 && $pl['delete'] == 0)
			{
				...
				}elseif($pl['spam']>0)
				{
					...
					$reimg = array();
					if(preg_match_all("/\[img\[(.*?)\]\]\[1\]/", $pl['text'], $reimg) && ($u->info['admin']>0 || $pl['type']==5 || $pl['type']==6))
					{
						$k = 0;
						while($k<=count($reimg[1]))
						{
							$pl['text'] = str_replace('[img['.$reimg[1][$k].']][1]','<img src=http://img.combatz.ru/i/'.$reimg[1][$k].' height=19>',$pl['text']);
							$k++;
						}
					}elseif(preg_match_all("/\[img\[(.*?)\]\]/", $pl['text'], $reimg) && ($u->info['admin']>0 || $pl['type']==5 || $pl['type']==6))
					{
						$k = 0;
						while($k<=count($reimg[1]))
						{
							$pl['text'] = str_replace('[img['.$reimg[1][$k].']]','<img src=http://img.combatz.ru/i/'.$reimg[1][$k].'>',$pl['text']);
							$k++;
						}
					}
